...
from setup_cosmological import z_p  # get real data for plotting purposes
from setup_cosmological import mu_u, mu_p  # get real data for plotting purposes
...
plot_synthetic_ground_truth(x=x, ground_truth=X.adjoint(ground_truth_field).val, x_max_pn=np.max(np.log(1+z_p)), save=False)
plot_synthetic_data(neg_scale_fac_mag=neg_a_mag, data=d.val, x_max_pn=np.max(np.log(1+z_p)),
                    mu_arrays=np.concatenate((mu_p, mu_u)), save=False)
