...
def NormalTransform(mean, sigma, key, N_copies=0, custom_domain=None):
...
def NormalTransform2(mean, sigma, key, N_copies=0):
    """Opchain that transforms standard normally distributed values to
    normally distributed values with given mean an standard deviation.

    So fluctuations doesnt log an `adder` apply call since fluctuations can be identified through the application
    of the pointwise function.
    ...
    domain = UnstructuredDomain(N_copies)
    mean, sigma = (value_reshaper(param, N_copies) for param in (mean, sigma))
    mean_adder = Adder2(makeField(domain, mean))
    sigma_op = DiagonalOperator(makeField(domain, sigma))
    return mean_adder @ sigma_op @ ducktape(domain, None, key)
...
def LognormalTransform(mean, sigma, key, N_copies):
    ...
    return NormalTransform2(logmean, logsigma, key, N_copies).ptw("exp")


def StandardUniformTransform(key, N_copies=0, upper_bound=1, shift=0):
    if N_copies == 0:
        domain = DomainTuple.scalar_domain()
        # mean, sigma = np.asarray(mean, dtype=float), np.asarray(sigma, dtype=float)
...
def ExponentialTransform(mu, key, N_copies=0):
    if N_copies != 0:
        raise ValueError("N_copies > 0 not implemented")
    U = StandardUniformTransform(key, N_copies)
    return -mu * U.ptw("log")