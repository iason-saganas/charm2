...
def read_data_union():
    ...
    path_to_data = f'{data_dir}/Union_2_1_data.txt'
    path_to_cov = f'{data_dir}/Union2_1_Cov-syst.txt'
...
def read_data_pantheon():
    ...
    path_to_data = f'{data_dir}/Pantheon+SH0ES.csv'
    path_to_cov = f'{data_dir}/Pantheon+SH0ES_STAT+SYS.txt'
...
def read_data_des():
    ...
    """
    print("\nReading DESY5 data...")
    filename = f"{data_dir}/DES-SN5YR_HD.csv"
    ...
    filename = f"{data_dir}/DES-SN5YR-STAT+SYS.txt.gz"
...
def store_meta_data(name, duration_of_inference, len_d, inference_type, signal_model_param,
    ...
    Args:
        ...
        folder_name: Where the intermediate output information from `optimize_kl` is stored.
    ...
    # Define paths
    temp_dir = f'{data_storage_dir_name}/pickled_inferences/{folder_name}'
    final_dir = f'{data_storage_dir_name}/pickled_inferences/{inference_type}'
...
def new_store_meta_data(name, duration_of_inference, len_d, inference_type, signal_model_param,
    ...
    final_dir = data_storage_dir_name
...
def plot_synthetic_ground_truth(x: RGSpace, ground_truth: np.ndarray, x_max_pn: float, show=True, save=False,
    ...
    if reconstruction is not None:
        ...
        # 1. uncertainty band (bottom)
        custom_ax.fill_between(
            signal_domain_reduced,
            signal_field_reduced - error_field_reduced,
            signal_field_reduced + error_field_reduced,
            color=light_blue,
            alpha=1,
            label=None
    ...
    if reconstruction is not None:
        ...
        # Needed to paste here again to get the order that I want for the plot

        signal_domain_reduced = remove_every_second(x, n=2)
        signal_field_reduced = remove_every_second(reconstruction[0], n=2)
        error_field_reduced = remove_every_second(reconstruction[1], n=2)

        # custom_ax.errorbar(x=signal_domain_reduced, y=signal_field_reduced, yerr=error_field_reduced, color=blue,
        #                    ecolor=light_blue,
...
def construct_initial_position(n_pix_ext, distances, fluctuations, apply_prior_line_slope=False,
    ...
    One last note: Here, I am drawing the xi_s values from a simple standard normal distribution.
    ...
    xi_values = np.loadtxt(f"{data_dir}/mean_posterior_union_xi_s.txt")
...
@dataclass
class _LhContainer:
    like: LikelihoodEnergyOperator
    meta: _LhMetaContainer
