...
def plot_charm2(posterior_samples, LH:_LhContainer, plot_domain:Literal["data", "signal", "extended signal"]="signal",
    ...
    if plot_domain == "extended signal":
        smpls = [field.val for field in posterior_signal_samples]
    elif plot_domain == "signal":
        smpls = [ZP.adjoint(field).val for field in posterior_signal_samples]
    ...
    plot_charm2_in_comparison_fields(x_max_pn=np.max(np.log(1 + z_p)), x_max_union=np.max(np.log(1 + z_u)),
                                     x_max_des=np.max(np.log(1 + z_d)), show=show, save=save, x=x.field().val,
