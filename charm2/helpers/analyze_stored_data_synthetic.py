...
for idx, fluct in enumerate(fluct_range):
    ...
    # pickled_data = unpickle_me_this("/Users/iason/PycharmProjects/Charm2/data_storage/PAPER_fluct_is_not_constrained/synthetic/PAPER_b0_0_2_uniform_data.pickle", absolute_path=True)
    pickled_data = unpickle_me_this(
        "/data_storage/PAPER_fluct_is_not_constrained/synthetic/PAPER_b0_0_2_exponential_data.pickle", absolute_path=True)
...
if plot_in_signal_space:
    ...
    if not full_signal_space:
        ...
        # mean_prior_array= np.mean([s(xi).val for xi in [ift.from_random(s.domain) for _ in range(10)]], axis=0)
        # mean_prior_field = ift.Field(domain=ift.DomainTuple.make(s.target), val=mean_prior_array)
